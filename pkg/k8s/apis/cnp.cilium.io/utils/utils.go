...
func parseToCiliumIngressRule(namespace string, inRule []v3.IngressRule) []v3.IngressRule {
	...
	for i, ing := range inRule {
		if ing.FromEndpoint != nil {
			retRule[i].FromEndpoint.ToPorts = ing.FromEndpoint.ToPorts.DeepCopy()

			es := v3.NewESFromK8sLabelSelector("", ing.FromEndpoint.EndpointSelector.LabelSelector)
			retRule[i].FromEndpoint.EndpointSelector = &es
			if retRule[i].FromEndpoint.EndpointSelector.MatchLabels == nil {
				retRule[i].FromEndpoint.EndpointSelector.MatchLabels = map[string]string{}
			}
			// There's no need to prefixed K8s
			// prefix for reserved labels
			if retRule[i].FromEndpoint.EndpointSelector.HasKeyPrefix(labels.LabelSourceReservedKeyPrefix) {
				continue
			}
			...
			if !retRule[i].FromEndpoint.EndpointSelector.HasKey(podPrefixLbl) {
				retRule[i].FromEndpoint.EndpointSelector.MatchLabels[podPrefixLbl] = namespace
			}
		}

		if ing.FromRequire != nil {
			retRule[i].FromRequire.ToPorts = ing.FromRequire.ToPorts.DeepCopy()

			es := v3.NewESFromK8sLabelSelector("", ing.FromRequire.EndpointSelector.LabelSelector)
			retRule[i].FromRequire.EndpointSelector = &es
			if retRule[i].FromRequire.EndpointSelector.MatchLabels == nil {
				retRule[i].FromRequire.EndpointSelector.MatchLabels = map[string]string{}
			}
			// The user can explicitly specify the namespace in the
			// FromEndpoints selector. If omitted, we limit the
			// scope to the namespace the policy lives in.
			if _, ok := retRule[i].FromRequire.EndpointSelector.MatchLabels[podPrefixLbl]; !ok {
				retRule[i].FromRequire.EndpointSelector.MatchLabels[podPrefixLbl] = namespace
			}
		}

		retRule[i].FromEntity = ing.FromEntity.DeepCopy()

		retRule[i].FromIPs = ing.FromIPs.DeepCopy()
	}
...
func parseToCiliumEgressRule(namespace string, inRule []v3.EgressRule) []v3.EgressRule {
	...
	for i, eg := range inRule {
		if eg.ToEndpoint != nil {
			retRule[i].ToEndpoint.ToPorts = eg.ToEndpoint.ToPorts.DeepCopy()

			es := v3.NewESFromK8sLabelSelector("", eg.ToEndpoint.EndpointSelector.LabelSelector)
			retRule[i].ToEndpoint.EndpointSelector = &es
			if retRule[i].ToEndpoint.EndpointSelector.MatchLabels == nil {
				retRule[i].ToEndpoint.EndpointSelector.MatchLabels = map[string]string{}
			}
			// There's no need to prefixed K8s
			// prefix for reserved labels
			if retRule[i].ToEndpoint.EndpointSelector.HasKeyPrefix(labels.LabelSourceReservedKeyPrefix) {
				continue
			}
			// The user can explicitly specify the namespace in the
			// ToEndpoints selector. If omitted, we limit the
			// scope to the namespace the policy lives in.
			if !retRule[i].ToEndpoint.EndpointSelector.HasKey(podPrefixLbl) {
				retRule[i].ToEndpoint.EndpointSelector.MatchLabels[podPrefixLbl] = namespace
			}
		}

		if eg.ToRequire != nil {
			retRule[i].ToRequire.ToPorts = eg.ToRequire.ToPorts.DeepCopy()

			es := v3.NewESFromK8sLabelSelector("", eg.ToRequire.EndpointSelector.LabelSelector)
			retRule[i].ToRequire.EndpointSelector = &es
			if retRule[i].ToRequire.EndpointSelector.MatchLabels == nil {
				retRule[i].ToRequire.EndpointSelector.MatchLabels = map[string]string{}
			}
			// The user can explicitly specify the namespace in the
			// ToEndpoints selector. If omitted, we limit the
			// scope to the namespace the policy lives in.
			if _, ok := retRule[i].ToRequire.EndpointSelector.MatchLabels[podPrefixLbl]; !ok {
				retRule[i].ToRequire.EndpointSelector.MatchLabels[podPrefixLbl] = namespace
			}
		}

		retRule[i].ToEntity = eg.ToEntity.DeepCopy()

		retRule[i].ToIPs = eg.ToIPs.DeepCopy()

		retRule[i].ToService = eg.ToService.DeepCopy()
	}
