...
func (i *IngressRule) sanitize() error {
	l3Members := map[string]int{
		//"FromEndpoint": len(i.FromEndpoint),
		//"FromIPs":     len(i.FromIPs),
		//"FromCIDRSet":  len(i.FromCIDRSet),
		//"FromEntity": len(i.FromEntity),
	}
	...
	//for n := range i.FromIPs {
	//	prefixLength, err := i.FromIPs[n].sanitize()
	//	if err != nil {
	//		return err
	//	}
...
func (e *EgressRule) sanitize() error {
	...
	//	//"ToEndpoint": len(e.ToEndpoint),
	//	"ToEntity": len(e.ToEntity),
	//	"ToService": len(e.ToService),
	//}
	//l3DependentL4Support := map[interface{}]bool{
	//	"ToCIDR":      false,
	//	"ToCIDRSet":   false,
