...
func Test_v2RuleTov3Rule(t *testing.T) {
	type args struct {
		v2Rule *v2.Rule
	}
	tests := []struct {
...
func Test_v2ServiceTov3Service(t *testing.T) {
	type args struct {
		v2S *v2.Service
	}
	tests := []struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := v2ServiceTov3Service(tt.args.v2S); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("v2ServiceTov3Service() = %v, want %v", got, tt.want)
			}
		})
	}
}
